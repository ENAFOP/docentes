			...
			if($user->isAdmin()) //añadido por Mario
				{
					...
					$rutaCharts="/out/out.Charts.php";
					 $txtpath .= "<li class=\"pull-right breadcrumb-btn\"><a href =\"".$rutaCharts. "\" id=\"proximas\" type=\"button\" class=\"btn bg-purple  btn-sm\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".getMLText("Estadísticas")."\"><i class=\"fa fa-bar-chart fa-4x\"></i> </a></li>";


				 $rutaTipologias="/out/out.Categories.php";
					 $txtpath .= "<li class=\"pull-right breadcrumb-btn\"><a href =\"".$rutaTipologias. "\" id=\"proximas\" type=\"button\" class=\"btn bg-orange btn-sm\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".getMLText("Gestionar tipologías")."\"><i class=\"fa fa-balance-scale fa-4x\"></i> </a></li>";



					$rutaProblemas="/out/out.GestionProblemasJuridicos.php";
					 $txtpath .= "<li class=\"pull-right breadcrumb-btn\"><a href =\"".$rutaProblemas. "\" id=\"proximas\" type=\"button\" class=\"btn bg-black btn-sm\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\""."Gestionar problemas jurídicos"."\"><i class=\"fa fa-university  fa-4x\"></i> </a></li>";

					 $txtpath .= "<li class=\"pull-right breadcrumb-btn\"><a id=\"add-document\" type=\"button\" class=\"btn bg-blue btn-sm\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".getMLText("Añadir resolución")."\"><i class=\"fa fa-plus fa-3x\"></i> <i class=\"fa fa-gavel fa-4x\"></i></a> </li>";

				}

				 $rutaTesauro="/out/out.Tesauro.php";
			  $txtpath .= "<li class=\"pull-right breadcrumb-btn\"><a href =\"".$rutaTesauro. "\" id=\"proximas\" type=\"button\" class=\"btn bg-maroon btn-sm\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".getMLText("Búsqueda por tesauro")."\"><i class=\"fa fa-sitemap fa-4x\"></i> </a></li>";

			  $rutaBusquedaAvanzada="/out/out.BusquedaAvanzada.php";
			  $txtpath .= "<li class=\"pull-right breadcrumb-btn\"><a href =\"".$rutaBusquedaAvanzada. "\" id=\"proximas\" type=\"button\" class=\"btn bg-gray btn-sm\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".getMLText("Búsqueda avanzada")."\"><i class=\"fa fa-search-plus  fa-4x\"></i> </a></li>";

			  $rutaBusquedaSimple="/out/out.BusquedaSimple.php";
			  $txtpath .= "<li class=\"pull-right breadcrumb-btn\"><a href =\"".$rutaBusquedaSimple. "\" id=\"proximas\" type=\"button\" class=\"btn bg-red btn-sm\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".getMLText("Búsqueda simple")."\"><i class=\"fa fa-search  fa-4x\"></i> </a></li>";
		}
...
<?php
	...
	function printAttributeEditField($attrdef, $attribute, $fieldname='attributes', $norequire=false) { /* {{{ */
		...
		case SeedDMS_Core_AttributeDefinition::type_date:
				$objvalue = NULL;//$attribute ? (is_object($attribute) ? $attribute->getValue() : $attribute) : '';
?>
...
<?php
	...
	function documentListRow($document, $previewer, $skipcont=false, $version=0) { /* {{{ */
		...
		if($latestContent) {
			...
			////////////////
			//Modificado por Mario para que en lugar de fecha de caducidad me muestre Fecha de resolución
			$fechaResolucion=obtenerParametro("Fecha de resolución",$document,$dms);
			$decision=obtenerParametro("Decisión",$document,$dms);
			$content .= "<br/><span style=\"font-size: 85%; font-style: italic; color: #666; \">".getMLText('fecha_resolucion').": <b>".htmlspecialchars($fechaResolucion)."</b>, decisión del TEG: <b>".$decision[0]."</b></span>";
...
//				$content .= "<td>".$version."</td>";
			...
			//quitado por Mario: no me es útil función de bloquear
			/*if($document->getAccessMode($user) >= M_READWRITE) 
			{
				$content .= $this->printLockButton($document, 'splash_document_locked', 'splash_document_unlocked', true);
			}*/
			if($enableClipboard) {
				$content .= '<a type="button" class="btn btn-success btn-sm addtoclipboard btn-action" rel="D'.$docID.'" msg="'.getMLText('splash_added_to_clipboard').'" data-toggle="tooltip" data-placement="bottom" title="'.getMLText("add_to_clipboard").'"><i class="fa fa-copy fa-2x"></i></a>';
			}
