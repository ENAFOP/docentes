...
class SeedDMS_View_Estadisticas extends SeedDMS_Bootstrap_Style 
{
 /**
 ...
 **/
  ...
  { /* {{{ */
    ...
    if($user->isAdmin())
    {
      $this->htmlStartPage("Ver postulación", "skin-blue sidebar-mini sidebar-collapse");
    }
    $this->containerStart();
    $this->mainHeader();
    $this->mainSideBar();
 ...
 $this->startBoxPrimary("Seleccione una fecha inicial y una final, de postulación, para mostrar estadísticas en ese período");
...
<?php $creationdate=FALSE ?>
    <form name="busquedaAvanzada" id="busquedaAvanzada" action="/out/out.MostrarEstadisticas.php" method="POST">
...
<label>Filtro: </label>
        <select class="form-control select"  name="filtro" id="filtro" required>
      <option disabled selected value>Seleccione un filtro</option>
      <option value="comparativo">Comparativa entre postulantes y aprobados</option>
     <option value="genero">Género</option>
     <option value="edad">Edad</option>
     <option value="profesion">Profesión</option>
     <option value="temas">Temas de la administración pública</option>
     <option value="docencia">Práctica de la docencia</option>
     <option value="metodologia">Metodología</option>
     <option value="extranjeros">Nacionales vs extranjeros</option>
